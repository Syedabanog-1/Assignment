...
• Make sure your comparison is case insensitive. If 'John' has been
 ...
 const current_Users: string [] = ['Tayyaba','Fatima','Malaika','Ayma','Shams','Pakiza','tahira'];
 const new_Users: string [] = ['Chandni','tayyaba','sunaira','Ayma','malaika','farah','roshni'];

const currentuserL: string[] = current_Users.map(user => user.toLowerCase());
for (const user of new_Users) {
  if(currentuserL.includes(user.toLowerCase())) {
